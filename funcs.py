...
def getData_ESruns(Epath = None, fn = None, opt = None):
    ...
    if opt is not None:
        ...
        lfn = len(fn)
        #tm_files = sorted([f for f in os.listdir(Epath) if f[:lfn] == fn and f[lfn+1:lfn+3] == 'tm'])
        ...
        for of in opt_files:
            ...
            final_x.append(opt.best_params()[0])
...
def plot_shots(Epath, fn, ax, ylim, rad = True):
    ...
    for of in opt_files:
        ...
        mean, var = opt.model.predict_noiseless(inputs)
        pos.append(inputs[np.argmax(mean), 0])
    
    pos = np.array(pos)
    ax.scatter(2*np.pi*pos, ylim*1.1*np.ones_like(pos), s = 60, marker = 'x',
               facecolors = 'k', zorder = 5)
    x = np.repeat(2*np.pi*pos, 2)
    y = np.vstack((ylim*1.1*np.ones_like(pos), np.zeros_like(pos))).reshape((-1,),order='F')
    #ax.plot(x, y, 'k', alpha = 1.0, zorder = 2)    
